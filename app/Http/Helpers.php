...
function chatofAccountName($id){
    $name = ChartofAccount::find($id);
    return $name->name;
}
function productName($id){
    ...
    if($OrderDetails)
    {
      return   $OrderDetails->product ? $OrderDetails->product->name : '';
    }else{
        return  '';
    }
...
{
    return VoucherItem::where('voucher_id',$id)->sum('debit');

}
function totalCredit($id)
{
    return VoucherItem::where('voucher_id',$id)->sum('credit');
...
if (!function_exists('sendSMS')) {
    function sendSMS($to, $from, $text)
    {
        ...
        if (OtpConfiguration::where('type', 'nexmo')->first()->value == 1) {
            ...
            return $response;
        }elseif (OtpConfiguration::where('type', 'robi_sms_credential')->first()->value == 1){
            $text = str_replace(" ", "%20", $text);

            $robi_sms_username = env("robi_sms_username"); //put robi_sms_username
            $robi_sms_password = env("robi_sms_password"); //put Password
            $robi_sms_from = env("robi_sms_from"); //put robi_sms_from

            $url = "https://api.mobireach.com.bd/SendTextMessage?Username={$robi_sms_username}&Password={$robi_sms_password}&From={$robi_sms_from}&To={$to}&Message={$text}";
        ...
        }elseif (OtpConfiguration::where('type', 'metrotel')->first()->value == 1){
            $url = 'https://24smsbd.com/api/bulkSmsApi';
            $data = array('sender_id' => env("METROTEL_SMS_SID"),
                'apiKey' => env("METROTEL_SMS_API_TOKEN"), //put ssl provided api_token here
                'mobileNo' => $to,
                'message' => $text
            );
...
if (!function_exists('home_price')) {
    function home_price($id)
    {
        ...
        if ($product->tax_type == 'percent') {
            $lowest_price_tax = ($lowest_price * $product->tax) / 100;
            $highest_price_tax = ($lowest_price * $product->tax) / 100;

            $lowest_price += $lowest_price_tax;
            $highest_price += $highest_price_tax;
        } elseif ($product->tax_type == 'amount') {
            $lowest_price_tax = $product->tax;
            $highest_price_tax = $product->tax;
            $lowest_price += $lowest_price_tax;
            $highest_price += $highest_price_tax;
        }
        ...
        if ($product->category->commision_rate) {
            if ($product->added_by == 'admin') {
                ...
                $lowest_price += ((($product->unit_price + $lowest_price_tax)) * $product->category->commision_rate) / 100;
                $highest_price += ((($product->unit_price + $highest_price_tax)) * $product->category->commision_rate) / 100;
            }

        }
        if ($product->shipping_cost) {
            $lowest_price += $product->shipping_cost;
            $highest_price += $product->shipping_cost;
    ...
    function home_price_1($id)
    {
        ...
        if ($product->tax_type == 'percent') {
            ...
            $lowest_price += (($product->unit_price - $price_discount) * $product->tax) / 100;
            $highest_price += (($product->unit_price - $price_discount) * $product->tax) / 100;
        } elseif ($product->tax_type == 'amount') {
        ...
        if ($product->category->commision_rate) {
            if ($product->added_by == 'admin') {
                ...
                $lowest_price += 0;
                $highest_price += 0;
                $lowest_price += (($product->unit_price - $product->discount) * $product->category->commision_rate) / 100;
                $highest_price += (($product->unit_price - $product->discount) * $product->category->commision_rate) / 100;
        ...
        if ($product->category->commision_rate) {
            if ($product->added_by == 'admin') {
                ...
                if ($product->discount_type == 'percent') {
                    $price_discount = ($product->unit_price * $product->discount) / 100;
                    $unit_price_discount = ($product->unit_price - $price_discount);
                    $lowest_price += ($unit_price_discount * $product->category->commision_rate) / 100;
                    $highest_price += ($unit_price_discount * $product->category->commision_rate) / 100;
                } else {
                    $lowest_price += (($product->unit_price - $product->discount) * $product->category->commision_rate) / 100;
                    $highest_price += (($product->unit_price - $product->discount) * $product->category->commision_rate) / 100;
...
if (!function_exists('home_discounted_price')) {
    if (!function_exists('home_discounted_price')) {
        function home_discounted_price_2($id)
        {
            $product = Product::findOrFail($id);
            $lowest_price = $product->unit_price;
            $highest_price = $product->unit_price;
            ...
            $lowest_price_with_discount = 0;
            $highest_price_with_discount = 0;
            if (!$inFlashDeal) {
                if ($product->discount_type == 'percent') {
                    $lowest_price -= ($lowest_price * $product->discount) / 100;
                    $lowest_price_with_discount = ($lowest_price * $product->discount) / 100;
                    $highest_price -= ($highest_price * $product->discount) / 100;
                    $highest_price_with_discount = ($highest_price * $product->discount) / 100;
                } elseif ($product->discount_type == 'amount') {
                    $lowest_price -= $product->discount;
                    $highest_price -= $product->discount;

                    $lowest_price_with_discount = $product->unit_price - $product->discount;
                    $highest_price_with_discount = $product->unit_price - $product->discount;
                }
            }
            ...
            if ($product->category->commision_rate) {
                if ($product->added_by == 'admin') {
                    ...
                    $lowest_price += ($lowest_price_with_discount * $product->category->commision_rate) / 100;
                    $highest_price += ($highest_price_with_discount * $product->category->commision_rate) / 100;
                }
        ...
        function home_discounted_price_3($id)
        {
            $product = Product::findOrFail($id);
            $lowest_price = $product->unit_price;
            $highest_price = $product->unit_price;
            ...
            if ($product->category->commision_rate) {
                if ($product->added_by == 'admin') {
                    ...
                    /*    $lowest_price += ($product->unit_price*$product->category->commision_rate)/100 ;
                        $highest_price += ($product->unit_price*$product->category->commision_rate)/100 ;*/
        ...
        function home_discounted_price($id)
        {
            ...
            foreach ($flash_deals as $flash_deal) {
                if ($flash_deal != null && $flash_deal->status == 1 && strtotime(date('d-m-Y')) >= $flash_deal->start_date && strtotime(date('d-m-Y')) <= $flash_deal->end_date && FlashDealProduct::where('flash_deal_id', $flash_deal->id)->where('product_id', $id)->first() != null) {
                    $flash_deal_product = FlashDealProduct::where('flash_deal_id', $flash_deal->id)->where('product_id', $id)->first();
                    if ($flash_deal_product->discount_type == 'percent') {
                        $lowest_price_discount = ($lowest_price * $flash_deal_product->discount) / 100;
                        $highest_price_discount = ($highest_price * $flash_deal_product->discount) / 100;
                        $lowest_price -= $lowest_price_discount;
                        $highest_price -= $highest_price_discount;
                    } elseif ($flash_deal_product->discount_type == 'amount') {
                        $lowest_price_discount = $flash_deal_product->discount;
                        $highest_price_discount = $flash_deal_product->discount;
                        $lowest_price -= $lowest_price_discount;
                        $highest_price -= $highest_price_discount;
                    }
                    $inFlashDeal = true;
            ...
            if (!$inFlashDeal) {
                if ($product->discount_type == 'percent') {
                    $lowest_price_discount = ($lowest_price * $product->discount) / 100;
                    $highest_price_discount = ($highest_price * $product->discount) / 100;
                    $lowest_price -= $lowest_price_discount;
                    $highest_price -= $highest_price_discount;
                } elseif ($product->discount_type == 'amount') {
                    $lowest_price_discount = $product->discount;
                    $highest_price_discount = $product->discount;
                    $lowest_price -= $lowest_price_discount;
                    $highest_price -= $highest_price_discount;
                }
            }
            ...
            if ($product->tax_type == 'percent') {
                $lowest_price_tax = ($lowest_price * $product->tax) / 100;
                $highest_price_tax = ($highest_price * $product->tax) / 100;
                $lowest_price += $lowest_price_tax;
                $highest_price += $highest_price_tax;
            } elseif ($product->tax_type == 'amount') {
                $lowest_price_tax = $product->tax;
            ...
            if ($product->category->commision_rate) {
                if ($product->added_by == 'admin') {
                    ...
                    $lowest_price += ((($product->unit_price + $lowest_price_tax) - $lowest_price_discount) * $product->category->commision_rate) / 100;
                    $highest_price += ((($product->unit_price + $highest_price_tax) - $highest_price_discount) * $product->category->commision_rate) / 100;
                }
...
if (!function_exists('home_base_price')) {
    ...
    //             $price += 0;
    //         } else {
    //             $price += ((($product->unit_price + $price_tax)) * $product->category->commision_rate) / 100;
    //         }
    ...
    function home_base_price_1($id)
    {
        ...
        if ($product->tax_type == 'percent') {
            $price_discount = ($product->unit_price * $product->discount) / 100;
            $price += (($product->unit_price - $price_discount) * $product->tax) / 100;
        } elseif ($product->tax_type == 'amount') {
            $price += $product->tax;
        }
//al
        ...
        if ($product->category->commision_rate) {
            if ($product->added_by == 'admin') {
                ...
                if ($product->discount_type == 'percent') {
                    $price_discount = ($product->unit_price * $product->discount) / 100;
                    $unit_price_discount = ($product->unit_price - $price_discount);
                    $price += ($unit_price_discount * $product->category->commision_rate) / 100;
                } else {
                    $price += (($product->unit_price - $product->discount) * $product->category->commision_rate) / 100;
                }
...
function home_discounted_base_pric5($id)
{
    $product = Product::findOrFail($id);
    $price = $product->unit_price;
...
function discount_perchent($id){
    if(is_int($id))
    {
        $home_discounted_base_price=home_discounted_base_pric5($id);
        $home_base_price=home_base_price5($id);
        return round(
            (ceil($home_base_price - $home_discounted_base_price)* 100)/$home_base_price);
    }
    return 0;
...
if (!function_exists('single_image')) {
    function single_image($id)
    {
        if (($asset = \App\Upload::find($id)) != null) {
            $array =  explode('/',$asset->file_name);
            if (!end($array)) {
               return 'product.jpg';
            }
            return end($array);
