...
class BusinessSettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        //echo '<pre>'; print_r($seller_registration); exit();
    ...
    public function shipping_configuration(Request $request){
        $shipping_configuration=ShippingSeller::paginate(4);
        return view('backend.setup_configurations.shipping_configuration.index',compact('shipping_configuration'));
    }

    //shipping configuration save function
    public function shipping_configuration_save(Request $request){
        $seller_shipping_type=$request->input('seller_shipping_type');

        if($seller_shipping_type == "free"){
            $seller_shipping_configuration = new ShippingSeller;
            $seller_shipping_configuration->title = $request->title;
            $seller_shipping_configuration->seller_shipping_type = $request->seller_shipping_type;
            $seller_shipping_configuration->seller_shipping_cost = 0;
            $seller_shipping_configuration->save();
            flash(translate('Information Saved successfully'))->success();
            return back();
        }else{
            ShippingSeller::create($request->all());
            flash(translate('Information Saved successfully'))->success();
            return back();
        }
    }

    //shipping configuration edit function
    public function shipping_configuration_edit($id){

        $shipping_configuration=ShippingSeller::find($id);
        return view('backend.setup_configurations.shipping_configuration.edit',compact('shipping_configuration'));
    }
    //shipping configuration edit function
    public function shipping_seller_configuration_update(Request $request){


            $shipping_configuration_data= new ShippingSeller;
            $shipping_configuration_data = $request->only(['title','seller_shipping_type']);
            //$seller_shipping_configuration['seller_shipping_cost'] = 0;
            ShippingSeller::where('id',$request->input('id'))->update($shipping_configuration_data);



        flash(translate('Information Update successfully'))->success();

        return redirect()->route('shipping_configuration.index');

    }

    //shipping configuration delete function
    public function shipping_configuration_delete($id){

        ShippingSeller::where('id', $id)->delete();;
