...
class ProductController extends Controller
{
    ...
    {
        $flash_deals = FlashDeal::where('status', 1)->where('featured', 1)->where('stock_or_flash',1)->where('start_date', '<=', strtotime(date('d-m-Y')))->where('end_date', '>=', strtotime(date('d-m-Y')))->get();
        return new FlashDealCollection($flash_deals);
    }
    public function stockOutOffer()
    {
        $flash_deals = FlashDeal::where('status', 1)->where('featured', 1)->where('stock_or_flash',2)->where('start_date', '<=', strtotime(date('d-m-Y')))->where('end_date', '>=', strtotime(date('d-m-Y')))->get();
        return new FlashDealCollection($flash_deals);
    }
