...
public function __construct()
    ...
    public function pay_to_seller(Request $request)
    {
        if($request->has('payment_id')){
            ...
            $payment_notification = SellerWithdrawRequest::where('viewed', 0)->get();

            foreach($payment_notification as $notification){
        ...
        elseif ($request->payment_option == 'bkash_payment') {
            return $this->seller_payment_done($request->session()->get('payment_data'), null);
        }
        elseif ($request->payment_option == 'nagad_payment') {
            return $this->seller_payment_done($request->session()->get('payment_data'), null);
        }
        else {
    ...
    public function seller_payment_done($payment_data, $payment_details){
        ...
        $payment = new Payment;
        $payment->seller_id = $seller->id;
        $payment->user_id = $seller->user_id;
        $payment->amount = $payment_data['amount'];
        $payment->payment_method = $payment_data['payment_method'];
        $payment->txn_code = $payment_data['txn_code'];
        $payment->pre_due_to_seller = $seller->admin_to_pay + $payment_data['amount'];
        $payment->cur_due_to_seller = $seller->admin_to_pay;
