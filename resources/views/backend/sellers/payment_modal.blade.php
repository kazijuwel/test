    ...
    <div class="modal-body">
      <table class="table table-striped table-bordered" >
          <tbody>
              ...
              @if ($seller->bkash_status == 1)
                  <tr>
                      <td>{{ translate('Bkash Number') }}</td>
                      <td>{{ $seller->bkash }}</td>
                  </tr>
              @endif
              @if ($seller->nagad_status == 1)
                  <tr>
                      <td>{{ translate('Nagad Number') }}</td>
                      <td>{{ $seller->nagad }}</td>
                  </tr>
              @endif
          ...
          <div class="form-group row">
              ...
              <div class="col-md-9">
                  <select name="payment_option" id="payment_option" class="form-control aiz-selectpicker" required>
                      ...
                      @endif
                      @if($seller->bkash_status == 1)
                          <option value="bkash_payment">{{translate('Bkash Payment')}}</option>
                      @endif
                      @if($seller->nagad_status == 1)
                          <option value="nagad_payment">{{translate('Nagad Payment')}}</option>
                      @endif
                  </select>
              </div>
          </div>
...
<script>
  $(document).ready(function(){
      $('#payment_option').on('change', function() {
        if (this.value == 'bank_payment' || this.value == 'cash'|| this.value == 'bkash_payment'|| this.value == 'nagad_payment')
        {
          $("#txn_div").show();
        }
        else
