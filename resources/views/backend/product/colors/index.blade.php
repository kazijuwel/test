		            ...
		            <tbody>
		                @foreach($colors as $key => $color)
		                    <tr>
		                        ...
		                        <td>{{ $color->name }}</td>
								<td>{{ $color->code }}</td>
								<td><span><span class='size-20px d-inline-block mr-2 rounded border' style='background:{{ $color->code }}'></span></span></td>
		                        <td class="text-right">
			...
			<div class="card-body">
				<form action="{{ route('colors.store') }}" method="POST">
					...
					<div class="form-group mb-3">
						...
						<input type="text" placeholder="{{translate('Color Name')}}" name="name" class="form-control" required>
					</div>
					
					<div class="form-group mb-3">
						<label for="name">{{translate('Hex Code')}}</label>
						<input type="text" class="form-control" name="code" placeholder="{{translate('Hex Code')}}">
					</div>
