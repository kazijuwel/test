            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header bg-secondary d-flex justify-content-between">
                        <span>
                            Purchase items of Purchase: {{ $purchase->id }} and Store/Warehouse : {{ $warehouse->store_name }}
                            <a href="" onclick="return printDiv('printArea');" class="btn btn-success btn-xs">Print</a>
                        </span>
                        <span><a href="{{route('warehousePurchaseList',['warehouse'=>$warehouse])}}" class="btn btn-warning btn-xs">Back</a></span>
                    </div>
                </div>
                ...
                <div class="card-body" id="printArea">
                    <div class="table-responsive">
                        <table class="table table-striped table-borderd table-sm text-nowrap">
                                ...
                                @foreach ($purchase_items as $purchase_item)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a class="printA" href="{{route('supplier.index',['supplier'=>$purchase_item->supplier_id])}}">{{ $purchase_item->supplier->user ? $purchase_item->supplier->name : $purchase_item->supplier->name }}</a>

                                            @else
                                            <a class="printA" href="{{route('sellers.index',['supplier'=>$purchase_item->supplier_id])}}">{{ $purchase_item->supplier->user ? $purchase_item->supplier->name : $purchase_item->supplier->name }}</a>
                                            @endif

                                        </td>
                                        <td><a href="{{route('products.all',['product'=>$purchase_item->product_id])}}" class="ptintA">{{ $purchase_item->product->name }}</a> </td>
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="7" class="text-right">Total Quantity: </td>
                                    <td colspan="1" class="text-right">{{ $purchase->total_quantity() }}</td>
                                </tr>
                                <tr>
                                    <td colspan="7" class="text-right">Unit Price/Purchase Price: </td>
