...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <h5 class="mb-0 h6">{{translate('Flash or Stock  Deal Information')}}</h5>
</div>
          ...
          <form class="p-4" action="{{ route('flash_deals.update', $flash_deal->id) }}" method="POST">
            @csrf
              ...
              <div class="form-group row">
                  <label class="col-lg-3 control-label" for="name">{{translate('Flash Or Stock Out')}}</label>
                  <div class="col-lg-9">
                      <select name="stock_or_flash" id="stock_or_flash" class="form-control aiz-selectpicker" required>


                          <option value="1" {{ $flash_deal->stock_or_flash == 1 ? 'selected' : '' }}>{{translate('Flash Deal')}}</option>
                          <option value="2" {{ $flash_deal->stock_or_flash == 2 ? 'selected' : '' }}>{{translate('Stock Out')}}</option>
                      </select>
                  </div>
              </div>
