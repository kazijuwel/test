...
Matchmaker Service in Bangladesh' }}">
    ...
    {{-- <link rel="stylesheet" --}}
        {{-- href="{{ asset('https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta2/css/fontawesome.min.css') }}" /> --}}
    ...
    <link rel="stylesheet" type="text/css"
        href="{{ asset('assets/select2-bootstrap4/dist/select2-bootstrap4.css') }}">
...
<body class="  {{ Auth::check() ? 'profile-page' : 'index-page' }}" {{-- data-url="{{ route('welcome.automail') }}" --}}>
    <div id="backtop">&#9650;</div>
...
<div class="fb-customerchat"
  ...
  page_id="1425864754148350">
    ...
    @auth
        ...
        @if (Auth::user()->hasIncompleteInformation())
            ...
            @elseif(Route::currentRouteName() == 'welcome.userSearch')
            @elseif(Route::currentRouteName() == 'welcome.username')

            @else
                @include('user.includes.modal.popUpModal')
    ...
    @include('user.includes.modal.userImageChange')
...
$(document).keydown(function(e){    
  if(e.which === 123){ return false;    }})
    ...
    <script>
        $(function() {
            $(document).on('submit', '.form-user-setting-for-edu', function(s) {
                s.preventDefault();
                var form = $(this),
    ...
    <script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-61a1e56239b06a82"></script>
