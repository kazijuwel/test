<div class=" aiz-category-menu bg-white rounded @if (Route::currentRouteName() == 'home') shadow-sm" @else shadow-lg" id="category-sidebar" @endif>
    ...
    no-scrollbar py-2 mb-0 text-left">
    @foreach (\App\Category::where('level', 0)->whereNotNull('priority')->orderBy('priority', 'asc')->get()->take(10)
    as $key => $category)
        <li class="category-nav-element" data-id="{{ $category->id }}">
            <a href="{{ route('products.category', $category->slug) }}" class="text-truncate text-reset  d-block"
