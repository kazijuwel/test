        ...
        <div class="px-2 py-4 px-md-4 py-md-3 bg-white rounded">
            <center>
                <div class="d-flex align-items-baseline">
                    <h5>
                        <b> {{ translate('Best Sell Products') }}</b>
                    </h5>
                    ...
                    <a href="{{ route('all_products', ['type' => 'best-sell-products']) }}" class="ml-auto mr-0 btn btn-primary btn-sm shadow-md">View More</a>
                </div>
            </center>
            ...
            {{-- <div class="aiz-carousel carosole2 gutters-10 half-outside-arrow" data-items="6" data-xl-items="5"
            ...
            {{-- Test Best Sell Products --}}

            <div class="pt-4" style="display:grid;grid-template-columns: 31% 33% 33%;grid-gap: 10px;">
                @foreach (filter_products(\App\Product::where('published', 1)->where('featured', '1'))->take(7)->get() as $key => $product)
                    @if ($loop->first)
                        <div class="mainDiv" style="position: relative;grid-row:1/4;height:100%;">
                            <a href="{{ route('product', $product->slug) }}" class="d-block" style="height:inherit">
                                <div class="card mb-0" style="height: inherit" >
                                    <div class="">
                                            <img class="text-center" src="{{ uploaded_asset($product->thumbnail_img) }}"
                                                alt="image" style="width: 100% ; position: relative;">
                                                @if (home_base_price($product->id) != home_discounted_base_price($product->id))
                                            <div class="discount"
                                                style="position: absolute; top: 0; right:0; margin: 8px; text-align: right; ">
                                                <img class="" src="{{ asset('public/images/discount.svg') }}" alt=""
                    ...
                    <div class="media-body">
                        <div class="p-md-3">
                            ...
                            <div class="aiz-p-hov-icon productcarthover" style="padding: 7px;position: absolute;top:0; right:40px;">
                                <a class="w3-animate-right" href="javascript:void(0)" onclick="addToWishList({{ $product->id }})" data-toggle="tooltip" data-title="{{ translate('Add to wishlist') }}" data-placement="left">
                                    <i class="la la-heart-o"></i>
                                </a>
                                <a class="w3-animate-right" href="javascript:void(0)" onclick="addToCompare({{ $product->id }})" data-toggle="tooltip" data-title="{{ translate('Add to compare') }}" data-placement="left">
                                    <i class="las la-sync"></i>
                                </a>
                                <a class="w3-animate-right" href="javascript:void(0)" onclick="showAddToCartModal({{ $product->id }})" data-toggle="tooltip" data-title="{{ translate('Add to cart') }}" data-placement="left">
                                    <i class="las la-shopping-cart"></i>
                                </a>
                            </div>
                        </div>
