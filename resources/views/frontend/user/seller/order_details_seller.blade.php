                    ...
                    <table class="table table-borderless table-responsive">
                        ...
                        @endphp
                            @foreach ($order->orderDetails->where('seller_id', Auth::user()->id) as $key => $orderDetail)
                                @php
                                    $subtotal +=  round( ($orderDetail->price/$orderDetail->quantity) +( $orderDetail->commision/$orderDetail->quantity)+ ($orderDetail->shipping_cost)) * $orderDetail->quantity;
                                @endphp
                                <tr>
                                    <td>{{ $key+1 }}</td>
                                    <td>
                                    ...
                                    <td>{{round( ($orderDetail->price/$orderDetail->quantity) +( $orderDetail->commision/$orderDetail->quantity)+ ($orderDetail->shipping_cost))}}</td>
                                    @if ($refund_request_addon != null && $refund_request_addon->activated == 1)
                                        <td>
                                    ...
                                    @if($status == 'delivered')
                                        <td>
                                            ...
                                            <p class="badge badge-inline badge-success text-center">{{ $orderDetail->received_status }}</p>
                                            @elseif($orderDetail->received_status == 'No')
                                            <p class="badge badge-inline badge-danger text-center">{{ $orderDetail->received_status }}</p>
                                            @endif
                                            @endif
                                        </td>
                                        @endif
                ...
                <div class="card-body pb-0">
                    <table class="table table-borderless">
                        <tbody>
                            ...
                            <tr>
                                <td class="w-50 fw-600">{{ translate('Delivery Charge')}}</th>
                                <td class="text-right">
                                    <span class="text-italic">{{ single_price($order->orderDetails->where('seller_id', Auth::user()->id)->sum('delivry_methods_charge')) }}</span>
                                </td>
                            </tr>
                            <tr>
