            ...
            <div class="card-header row gutters-5">
                <div class="aiz-titlebar text-left mt-2 mb-3">
                    <div class="align-items-center">
                        <h1 class="h3">{{translate('Edit Brand Information')}}</h1>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-body p-0">
            <ul class="nav nav-tabs nav-fill border-light">
  				@foreach (\App\Language::all() as $key => $language)
  					<li class="nav-item">
  						<a class="nav-link text-reset @if ($language->code == $lang) active @else bg-soft-dark border-light border-left-0 @endif py-3" href="{{ route('seller_edit_brand', ['id'=>$brand->id, 'lang'=> $language->code] ) }}">
            ...
            <form class="p-4" action="{{ route('brand_update', $brand->id) }}" method="POST" enctype="multipart/form-data">
