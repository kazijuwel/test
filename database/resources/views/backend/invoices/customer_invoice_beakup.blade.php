            ...
            @foreach ($order->orderDetails as $key => $orderDetail)
                @if ($orderDetail->product != null)
                    <tr class="">
                        ...
                        <td class="gry-color currency">{{ single_price(($orderDetail->price/$orderDetail->quantity) + $orderDetail->commision) }}</td>
                        <td class="gry-color currency">{{ single_price($orderDetail->tax/$orderDetail->quantity) }}</td>
                        <td class="text-right currency">{{ single_price($orderDetail->price+$orderDetail->tax + $orderDetail->commision) }}</td>
                    </tr>
                @endif
            @endforeach
            </tbody>
    ...
    <div style="padding:0 1.5rem;">
        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
            ...
            <tr>
                ...
                <td class="currency">{{ single_price($order->orderDetails->sum('price') + $order->orderDetails->sum('commision')) }}</td>
            </tr>
            <tr>
                <th class="gry-color text-left">{{ translate('Delivery Charge') }}</th>
                <td class="currency">{{ single_price($order->orderDetails->sum('delivry_methods_charge')) }}</td>
            </tr>
