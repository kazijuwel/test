...
<div class="aiz-titlebar text-left mt-2 mb-3">
	<div class="row align-items-center">
		<div class="col-md-6">
			<h1 class="h3">{{translate('All Flash or Stock Deals')}}</h1>
		</div>
		<div class="col-md-6 text-md-right">
			<a href="{{ route('flash_deals.create') }}" class="btn btn-circle btn-info">
				<span>{{translate('Create New Flash or Stock Deal')}}</span>
			</a>
		</div>
	</div>
</div>
...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0 h6">{{translate('Flash or Stock deals')}}</h5>
        ...
        <table class="table aiz-table mb-0" >
            <thead>
                <tr>
                    ...
                    <th>{{ translate('stock_or_flash') }}</th>
            ...
            <tbody>
                @foreach($flash_deals as $key => $flash_deal)
                    <tr>
                        ...
                        <?php
                            $deal_name = ' ';
                            if($flash_deal->stock_or_flash == 1){
                                $deal_name = 'Flash Sale';
                            }
                            elseif($flash_deal->stock_or_flash == 2){
                                $deal_name = 'Stock Out';
                            }
                        ?>
                        <td>{{$deal_name}}</td>
						...
						<td>
							<label class="aiz-switch aiz-switch-success mb-0">
								<input onchange="update_flash_deal_feature(this,'{{$flash_deal->stock_or_flash}}')" value="{{ $flash_deal->id }}" type="checkbox" <?php if($flash_deal->featured == 1) echo "checked";?> >
								<span class="slider round"></span>
							</label>
						</td>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_flash_deal_feature(el,stock_or_flash_id){
            ...
            $.post('{{ route('flash_deals.update_featured') }}', {_token:'{{ csrf_token() }}', id:el.value, featured:featured,stock_or_flash_id:stock_or_flash_id}, function(data){
