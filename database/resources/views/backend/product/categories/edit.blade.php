    ...
    <div class="card">
        <div class="card-body p-0">
            ...
            <form class="p-4" action="{{ route('categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
                ...
                @endif
                <div class="form-group row">
                    <label class="col-md-3 col-form-label">{{translate('Home Navbar Priority')}}</label>
                    <div class="col-md-9">
                        <input type="text" name="priority" value="{{ $category->priority }}" class="form-control" id="" placeholder="{{translate('Navbar Order Number')}}">
                    </div>
                </div>
