...
<p style="color: black;">Dear {{ $shipping_address->name }}</p>

<p style="color: black;">{{$order_confirmation_mail}}</p>

<p style="color: black;">Your Order Number is: {{ $order->code }}</p>

<p style="color: black;">Product Name:
@foreach ($order->orderDetails as $key => $orderDetail)
	@if ($orderDetail->product != null)

{{ $orderDetail->product->getTranslation('name') }} @if($orderDetail->variation != null) ({{ $orderDetail->variation }})</p> @endif
@endif
,
@endforeach

<p style="color: black;">Total Amount: {{ single_price($order->grand_total) }}</p>
